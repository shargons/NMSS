...
SELECT 
      ...
      ,[npe5__Status__c]
      ,[Role__c]
INTO [CFG_NMSS_PREPROD].dbo.npe5__Affiliation__c_LOAD
FROM [NMSS_SRC].[CFG_NMSS_QA].[dbo].[vw_DW_SFDC_Affiliation] C
INNER JOIN [NMSS_SRC].[SFINTEGRATION].[dbo].[XREF_Account_Org] AL
...
/******* DBAmp Insert Script *********/
EXEC SF_TableLoader 'Insert:BULKAPI2','CFG_NMSS_PREPROD','npe5__Affiliation__c_LOAD'
...
INSERT INTO [NMSS_SRC].[SFINTEGRATION].[dbo].[XREF_Affiliation]
           ([SFID]
           ,[DWID]
           ,[SystemOfCreation]
           ,[LastActionTaken]
...
SELECT
 ID
,[Data_Warehouse_ID__c]
INTO [CFG_NMSS_PREPROD].[dbo].[npe5__Affiliation_Lookup]
FROM npe5__Affiliation__c_LOAD_Result
WHERE Error = 'Operation Successful.'