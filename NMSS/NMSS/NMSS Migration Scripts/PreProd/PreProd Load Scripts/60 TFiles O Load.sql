...
--====================================================================
DROP TABLE ContentVersion_ROI_O_Insert


SELECT 
  NULL AS ID
 ,LEFT(FileName,80) as Legacy_ID__c
 ,REPLACE(FilePath,'Y:','‪D:') AS PathOnClient
 ,1 as IsMajorVersion
 ,REPLACE(FileName,'?','') as Title
 ,'P' as PublishStatus
 ,'N' as SharingPrivacy
 ,REPLACE(FilePath,'Y:','‪D:') as VersionData
 ,B.ID AS FirstPublishLocationId -- Get id from Consent_for_Release_of_Information__c_TFilesA_Insert
 INTO ContentVersion_ROI_O_Insert
 FROM [CFG_NMSS_PREPROD].[dbo].[T_ROIFiles] A
  INNER JOIN ROI_TFiles_Lookup B
 ON LEFT(FileName,80) = B.Legacy_ID__c
 WHERE FilePath LIKE '%\O\%'

 SELECT * FROM ContentVersion_ROI_O_Insert
  where FirstPublishLocationId IS NULL

UPDATE A
SET A.PathOnClient = REPLACE(PathOnClient,'?','')
FROM ContentVersion_ROI_O_Insert A
...
--====================================================================
/******* Change ID Column to nvarchar(18) *********/
ALTER TABLE ContentVersion_ROI_O_Insert
ALTER COLUMN ID NVARCHAR(18)

/******* DBAmp Insert Script *********/
EXEC SF_TableLoader 'Insert:soap,batchsize(1)','CFG_NMSS_PREPROD','ContentVersion_ROI_O_Insert'

SELECT *
--INTO ContentVersion_ROI_F_Insert_2
FROM ContentVersion_ROI_O_Insert_Result 
where Error <> 'Operation Successful.'


select count(*), Error from ContentVersion_ROI_O_Insert_Result GROUP BY Error
...
INSERT INTO ContentVersion_DELETE(Id) SELECT Id FROM ContentVersion_ROI_N_Insert_Result WHERE Error = 'Operation Successful.'
...
--====================================================================
 ...
 -- Case Files 
 DROP TABLE ContentVersion_CaseFile_O_Insert
...
SELECT
 ...
 INTO ContentVersion_CaseFile_O_Insert
 FROM [CFG_NMSS_PREPROD].[dbo].[T_CaseFiles] A
...
UPDATE A
SET A.PathOnClient = REPLACE(PathOnClient,'?','')
FROM ContentVersion_CaseFile_O_Insert A
...
--====================================================================
/******* Change ID Column to nvarchar(18) *********/
ALTER TABLE ContentVersion_CaseFile_O_Insert
ALTER COLUMN ID NVARCHAR(18)

SELECT * FROM ContentVersion_CaseFile_O_Insert
 where FirstPublishLocationId IS NULL

/******* DBAmp Insert Script *********/
EXEC SF_TableLoader 'Insert:soap,batchsize(1)','CFG_NMSS_PREPROD','ContentVersion_CaseFile_O_Insert'

SELECT * FROM ContentVersion_CaseFile_O_Insert_Result where Error <> 'Operation Successful.'

SELECT * FROM ContentVersion_CaseFile_N_Insert_Result where Error LIKE '%part%'

select count(*), Error from ContentVersion_CaseFile_N_Insert_Result GROUP BY Error