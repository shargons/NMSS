...
left join [TommiQA1].dbo.apfx_InteractionDetailResponseType rt on rt.interactiondetailid = id.interactiondetailid and rt.ActiveFlag = 1
left join [TommiQA1].dbo.apfx_refResponseType rr on rt.ResponseTypeId = rr.ResponseTypeId
left join [TommiQA1].dbo.apfx_refResponseCategory rc on rr.ResponseCategoryId = rc.ResponseCategoryId
left join [TommiQA1].dbo.[apfx_InteractionDetailTrackedTopic] tt on tt.InteractionDetailId = id.InteractionDetailId
join [TommiQA1].dbo.apfx_entity e on e.entityid = i.ConstituentId and e.ActiveFlag = 1 and entitycategorycode = 'I'
join [TommiQA1].dbo.apfx_sysInteractionDetailType dt on dt.DetailTypeCode = id.DetailTypeCode
