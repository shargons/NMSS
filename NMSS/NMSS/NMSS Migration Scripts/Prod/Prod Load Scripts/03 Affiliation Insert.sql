...
SELECT 
      ...
      ,[npe5__Status__c]
      ,[Role__c]
--INTO [CFG_NMSS_PROD].dbo.npe5__Affiliation__c_LOAD
FROM [CFG_NMSS_PROD].[dbo].[vw_DW_SFDC_Affiliation] C
INNER JOIN [dbo].[XREF_Account_Org] AL
ON C.[Source_npe5__Organization__c] = AL.DWID
INNER JOIN [dbo].[XREF_Contact] CL
ON C.[Source_npe5__Contact__c] = CL.DWID
WHERE C.ID IS NULL

/******* Check Load table *********/
UPDATE npe5__Affiliation__c_LOAD
SET Id = NULL
WHERE Id = 'NULL'

select * from npe5__Affiliation__c_LOAD
...
/******* DBAmp Insert Script *********/
EXEC SF_TableLoader 'Insert:BULKAPI','CFG_NMSS_PROD','npe5__Affiliation__c_LOAD'
...
--====================================================================
INSERT INTO [CFG_NMSS_PREPROD].[dbo].[XREF_Affiliation]
           ([SFID]
           ,[DWID]
           ,[SystemOfCreation]
           ,[LastActionTaken]
...
INSERT INTO [NMSS_PRD].[SFINTEGRATION].[dbo].[XREF_Affiliation]
           ([SFID]
           ,[DWID]
           ,[SystemOfCreation]
           ,[LastActionTaken]
...
SELECT
 ID
,[Data_Warehouse_ID__c]
INTO [CFG_NMSS_PROD].[dbo].[npe5__Affiliation_Lookup]
FROM npe5__Affiliation__c_LOAD_Result
WHERE Error = 'Operation Successful.'