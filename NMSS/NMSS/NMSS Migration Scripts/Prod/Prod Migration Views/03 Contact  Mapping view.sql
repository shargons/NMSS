...
FROM [CFG_NMSS_PROD].dbo.tbl_Qualified_Entity AS qe with (nolock) -- used for NMSS Navigator Individuals
INNER JOIN [Tommiprd1].[dbo].apfx_Entity AS E with (nolock)
	ON E.EntityId = qe.EntityId
LEFT JOIN [Tommiprd1].[dbo].apfx_EntityAddress AS EA with (nolock)
	ON EA.EntityId = E.EntityId AND EA.ActiveFlag = 1 AND EA.PrimaryFlag = 1
LEFT JOIN [Tommiprd1].[dbo].apfx_Address AS A with (nolock)
	ON A.AddressId = EA.AddressId
LEFT JOIN [Tommiprd1].[dbo].apfx_refStateProvince AS rSP with (nolock)
	ON rSP.StateProvinceId = A.StateProvinceId AND rSP.CountryId = A.CountryId
LEFT JOIN [Tommiprd1].[dbo].apfx_refCountry AS rC with (nolock)
	ON rC.CountryId = A.CountryId
LEFT JOIN [Tommiprd1].[dbo].[apfx_refCounty] rcty with (nolock)
	ON rcty.CountyId = a.CountyId
LEFT JOIN [Tommiprd1].[dbo].apfx_refReferralSource AS rRS with (nolock)
...
LEFT JOIN [Tommiprd1].[dbo].apfx_Chapter AS C_local with (nolock)
	ON C_local.ChapterCode = E.LocalChapterCode
LEFT JOIN [Tommiprd1].[dbo]. apfx_Chapter AS C_dm with (nolock)
	ON C_dm.ChapterCode = E.DirectMailChapterCode
LEFT JOIN [Tommiprd1].[dbo].apfx_refAddressType AS rAT with (nolock)
	ON rAT.AddressTypeCode = EA.AddressTypeCode
LEFT JOIN [Tommiprd1].[dbo].apfx_refGender AS rG with (nolock)
	ON rG.GenderCode = E.GenderCode
LEFT JOIN [Tommiprd1].[dbo].apfx_refEmployerType AS rET with (nolock)
	ON rET.EmployerTypeId = E.EmployerTypeId
LEFT JOIN [Tommiprd1].[dbo].apfx_refEmploymentStatus AS rES with (nolock)
	ON rES.EmploymentStatusId = E.EmploymentStatusId
--LEFT JOIN [Tommiprd1].[dbo].apfx_refEthnicity AS rE with (nolock)
...
LEFT JOIN [Tommiprd1].[dbo].apfx_refLivingSituation AS rLS with (nolock)
	ON rLS.LivingSituationId = E.LivingSituationId
LEFT JOIN [Tommiprd1].[dbo].apfx_refMaritalStatus AS rMS with (nolock)
	ON rMS.MaritalStatusCode = E.MaritalStatusCode
--LEFT JOIN [Tommiprd1].[dbo].apfx_refOccupation AS rO with (nolock)
...
--LEFT JOIN [Tommiprd1].[dbo].apfx_refHealthCoverage AS rHC_prim with (nolock)----
--	ON rHC_prim.HealthCoverageId = E.PrimaryHealthCoverageId------
LEFT JOIN [Tommiprd1].[dbo].apfx_refCoverageType AS rCT_prim with (nolock)
	ON rCT_prim.CoverageTypeId = chc.CoverageType
--LEFT JOIN [Tommiprd1].[dbo].apfx_refHealthCoverage AS rHC_sec with (nolock)
--	ON rHC_sec.HealthCoverageId = E.SecondaryHealthCoverageId
--LEFT JOIN [Tommiprd1].[dbo].apfx_refCoverageType AS rCT_sec with (nolock)
--	ON rCT_sec.CoverageTypeId = rHC_sec.CoverageType
--LEFT JOIN [Tommiprd1].[dbo].apfx_refIncomeSource AS rIS with (nolock)
--	ON rIS.IncomeSourceId = E.PrimaryIncomeSourceId
LEFT JOIN [Tommiprd1].[dbo].[vw_NMSS_help_ConstituentIncomeSources] as rIS with (nolock)
	ON rIS.ConstituentId = E.EntityId
LEFT JOIN [Tommiprd1].[dbo].apfx_refLanguage AS rL_prim with (nolock)
	ON rL_prim.LanguageId = E.PrimaryLanguageId
LEFT JOIN [Tommiprd1].[dbo].apfx_refLanguage AS rL_sec with (nolock)
	ON rL_sec.LanguageId = E.SecondaryLanguageId
LEFT JOIN [Tommiprd1].[dbo].apfx_XrefEntity AS XE with (nolock)
...
--LEFT JOIN [Tommiprd1].[dbo].apfx_Entity_MergeHistory as m with (nolock) -- Left join for #940
...
LEFT JOIN [Tommiprd1].[dbo].[apfx_refMilitaryStatus] mil on mil.MilitaryStatusId = e.MilitaryStatusId
