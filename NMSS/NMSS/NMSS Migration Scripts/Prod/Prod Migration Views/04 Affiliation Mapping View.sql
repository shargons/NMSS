...
FROM 
	...
	LEFT JOIN [Tommiprd1].dbo.apfx_EntityRelationship as ER with(nolock)
	...
	JOIN [Tommiprd1].dbo.apfx_refRelationshipType as rRT1  with(nolock)
		ON ER.RelationshipTypeId = rRT1.RelationshipTypeId
	JOIN [Tommiprd1].dbo.apfx_refRelationshipType as rRT2  with(nolock)
		ON rRT1.ReciprocalRelationshipTypeId = rRT2.RelationshipTypeId
	JOIN [Tommiprd1].dbo.apfx_EntityRelationship AS ER2 with(nolock)
	...
	JOIN [Tommiprd1].dbo.apfx_Entity as E1  with(nolock)
		ON ER.EntityId = E1.EntityId
	JOIN [Tommiprd1].dbo.apfx_Entity as E2  with(nolock)
