...
CREATE or ALTER VIEW [dbo].[vw_DW_CFG_DirectFARequest]  
AS
...
, Decision_Date__c
, Status__c 
, MAX(Requested_Amount__c) AS Requested_Amount__c
...
, CASE WHEN MAX(Benefit_Type__c) IN ('Aids for Daily Living','Assistive Technology Devices - High Tech','Auto Modification','Home Modification','Ramp','Reacher/Grabber','Stair Glide') THEN 'Access Mods'
     ...
     ELSE 'Other' END                                                       as Spend_Category__c  
, OwnerId
, CreatedById
FROM 
(
	...
	, CASE WHEN cp.CompleteFlag = 1 THEN 'FA Completed'
		   ELSE 'Active FA Payments' 
	  END																    as Status__c
	, IIF(vc.KeyProcessValueCategoryName = 'Requested Financial Benefit', pv.DollarAmount,NULL)	as Requested_Amount__c 
	, IIF(vc.KeyProcessValueCategoryName = 'Awarded Financial Benefit' and cp.CompleteFlag = 1 ,amount.Agg_DollarAmount,pv.DollarAmount)  as Award_Amount__c 
	, 'Direct'																as Award_Type__c
	...
	, IIF(vt.AccountingSegment IS NOT NULL,vt.KeyProcessValueTypeName,NULL) as Benefit_Type__c
	...
	(SELECT csp.ConstituentKeyProcessId,STRING_AGG(CAST(csp.Comments AS NVARCHAR(MAX)),char(10)) as Agg_Comments 
		FROM [TommiPrd1].[dbo].apfx_constituentkeyprocessstep csp 
	...
	(SELECT csv.ConstituentKeyProcessId,SUM(csv.dollaramount) as Agg_DollarAmount 
		FROM [TommiPrd1].[dbo].apfx_ConstituentKeyProcessValue csv
		     JOIN [TommiPrd1].[dbo].apfx_refKeyProcessValueType vt 
...
--WHERE X.DWUI_Interaction_ID__c = 152110281
