...
CREATE OR ALTER VIEW [vw_DW_CFG_ResponseType_Interest] AS
SELECT 
NULL																AS [Id] -- Salesforce ID
,'ResponseType'+CAST(ResponseTypeId AS NVARCHAR(50))				AS [DWUI_Response_Type_ID__c] -- nvarchar(120)
,ResponseTypeName													AS [Name] -- nvarchar(121)
,'ResponseCategory'+CAST(ResponseCategoryId AS NVARCHAR(50))		AS [Source_cfg_Interests__ParentInterest__c] -- Parent Interest
...
,RT.UpdatedDate													    AS [LastModifiedDate]
	,CASE 
	 ...
	 ,0													AS IsActive__c 	
FROM [Tommiprd1].[dbo].[apfx_refResponseType] RT 
