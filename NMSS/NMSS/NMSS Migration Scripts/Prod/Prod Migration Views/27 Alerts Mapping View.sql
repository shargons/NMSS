...
CREATE OR ALTER View [dbo].[vw_DW_SFDC_Warning__c] AS   
  ...
  FROM 
   [TommiPrd1].[dbo].apfx_ConstituentWarning AS CW  WITH(NOLOCK)   
  JOIN [TommiPrd1].[dbo].apfx_Entity AS E  WITH(NOLOCK)  
  ...
  LEFT JOIN [CFG_NMSS_PROD].dbo.tbl_Qualified_Entity AS QE on qe.EntityId = e.EntityId
 ...
 UNION
  ...
  JOIN [TommiPrd1].[dbo].apfx_ConstituentWarning AS CW  WITH(NOLOCK)  
