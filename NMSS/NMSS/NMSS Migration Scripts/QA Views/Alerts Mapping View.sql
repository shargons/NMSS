...
CREATE OR ALTER View [dbo].[vw_DW_SFDC_Warning__c] AS   
  ...
  LEFT JOIN [CFG_NMSS_QA].dbo.XREF_DW_SFDC_Warning__c AS XREF WITH(NOLOCK)  
   ON XREF.DWID = CW.ConstituentWarningId
  LEFT JOIN [CFG_NMSS_QA].dbo.XREF_Contact XC on XC.DWID = e.EntityId
  LEFT JOIN [CFG_NMSS_QA].dbo.tbl_Qualified_Entity AS QE on qe.EntityId = e.EntityId
  WHERE E.EntityCategoryCode = 'I'
  and cw.ActiveFlag = 1
