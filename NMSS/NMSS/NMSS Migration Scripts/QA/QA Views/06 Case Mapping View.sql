...
SELECT DISTINCT
	...
	,CASE
		WHEN cp.CompleteFlag = 1 AND id.DetailTypeCode IN ('F','B','C') THEN 'Closed'
		WHEN cp.CompleteFlag = 0 AND id.DetailTypeCode IN ('C') THEN 'Case Management'
		WHEN ID.DetailTypeCode NOT IN ('F','B','C') AND i.CompletedFlag = 0 THEN 'Closed'
		ELSE 'Service Navigation'
	 END																				AS [Status] -- Picklist
	,CASE WHEN DATEADD(hour, 12, DATEDIFF(DAY, 0, I.InteractionDateTime)) > cp.CompleteDate THEN cp.createdDate
	      ELSE DATEADD(hour, 9, DATEDIFF(DAY, 0, I.InteractionDateTime))
		  END AS [CreatedDate] -- DateTime
	,I.Comments																			AS [Description] -- Long Text
	,CASE WHEN id.DetailTypeCode = 'F' THEN 'DFA'
		  WHEN id.DetailTypeCode = 'B' THEN 'B&E'
		  WHEN id.DetailTypeCode = 'C' THEN 'Case Management'
	      ELSE I.description
	END																					AS [Subject] --  Text
	,CASE 
