...
FROM TommiQA1.dbo.apfx_Interaction I
	...
	LEFT JOIN TommiQA1.dbo.apfx_InteractionDetailReportableSituation rs
			on rs.InteractionDetailId = id.InteractionDetailId
	LEFT JOIN [CFG_NMSS_QA].[dbo].[Case] Cx
		ON Cx.Data_Warehouse_ID__c = id.InteractionId
	LEFT JOIN TommiQA1.dbo.apfx_refRiskToSelfScale ss
	    ON ss.RiskToSelfScaleId = id.RiskToSelfScaleId
	LEFT JOIN TommiQA1.dbo.apfx_refRiskToOthersScale os
	    ON os.RiskToOthersScaleId = id.RiskToOthersScaleId
	LEFT JOIN TommiQA1.dbo.apfx_refMentalStatusRating ms
	    ON ms.MentalStatusRatingId = id.MentalStatusRatingId
	LEFT JOIN TommiQA1.dbo.apfx_refReportableSituation rep
