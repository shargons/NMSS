...
SELECT 
	...
	,U.UserName												   AS [Email] -- nvarchar(128)
	...
	,U.UserName												   AS [Username] -- nvarchar(18)
	,UR.ID																   AS [UserRoleId] --nvarchar(18)
	,'Standard'															   AS [UserType]
FROM 
[NMSS_PRD].[Tommiprd1].dbo.apfx_secUser U
	LEFT JOIN [NMSS_PRD].[Tommiprd1].[dbo].[apfx_secUserRole] R ON U.UserId = R.UserId
	LEFT JOIN [CFG_NMSS_PROD].[dbo].[User] SFU ON SFU.Username = U.UserName
	LEFT JOIN [CFG_NMSS_PROD].[dbo].[Profile] P ON P.Name = 'Navigator'
	LEFT JOIN [CFG_NMSS_PROD].[dbo].[UserRole] UR ON UR.DeveloperName = 'MS_Navigator_Standard'
WHERE R.RoleId = 21 
		AND U.ActiveFlag = 1 
